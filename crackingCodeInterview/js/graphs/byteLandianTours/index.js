...
function createGraph() {
  class Graph {
    ...
    createNewRoads = () => {
      ...
      */
      Object.keys(this._vertices).forEach((vertex) => {              // iterate accross each vertex.
        ...
        for (let i = 0; i < length; i++) {
          if (this._vertices[vertex].newNeighbors.length) {          // because the array is being dynamically resized between iterations, we need to verify the array still has a length.
            ...
            this._vertices[vertex].newNeighbors.splice(this._vertices[vertex].newNeighbors.indexOf(v), 1); // remove the target vertex from the current vertice list.
          }
        }
...
const parseInputs = (input) => {
  ...
  let line = 1;
  for (let i = 0; i < testCases; i++) {     // iterate for each test case.
    ...
    for (let j = 0; j < cityCount - 1; j++) {    // iterate across each city.
